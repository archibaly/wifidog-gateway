...
fw_sync_with_authserver(void)
{
    ...
    for (p1 = p2 = worklist; NULL != p1; p1 = p2) {
        ...
        if (config->auth_servers != NULL) {
            auth_server_nrequest(&authresponse, REQUEST_TYPE_COUNTERS, p1->ip, p1->mac, p1->token, wifidog_cfg_version,
                                 p1->counters.incoming, p1->counters.outgoing, p1->counters.incoming_delta, p1->counters.outgoing_delta);
        }
